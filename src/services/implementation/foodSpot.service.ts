...
export class FoodSpotService implements IFoodSpotService{
    ...
    async getAllFoodSpots(page:number,limit:number,search:string): Promise<{ data: IFoodSpotResDto[],totalCount:number }> {
        try {
            const skip=(page-1)*limit;
            const spots=await this._foodSpotRepository.findAllFoodSpots(search,skip,limit,'foodie')
