...
export interface IBlogRepository extends IBaseRepository<IBlogDocument> {
    ...
    getAllBlogs(search: string, skip: number, limit: number, from: string, filter?: string | string[], isBlocked?: boolean): Promise<{ datas: IBlogDocument[] | null, totalCount: number }>;
