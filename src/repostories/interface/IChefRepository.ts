...
export interface IChefRepository extends IBaseRepository<IChefDocument> {
    ...
    findAllChefs(skip: number, limit: number, search: string, filter?: string): Promise<{ datas: IChefDocument[]; totalCount: number }>;
    findDetailsByChefId(chefId: string): Promise<IChefDocument | null>;
}